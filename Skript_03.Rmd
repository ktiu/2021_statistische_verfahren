...
Die Berechnung von Maßzahlen ([Sitzung 2](maßzahlen.html)) vereinfacht es uns, auch große Verteilungen miteinander zu vergleichen. Voraussetzung dafür ist jedoch, dass die Kennwerte (wie arithmetisches Mittel, Standardabweichung) in derselben Maßeinheit (kg, cm, °C, etc.) vorliegen und einen vergleichbaren Maßstab haben.
...
Mit den bekannten Methoden ([Sitzung 2](#einleitende-bemerkungen)) können wir das arithmetische Mittel $\bar{x}\approx 999,37$ und die Standardabweichung $s\approx8,56$ der Stichprobe bestimmen. Durch einsetzen dieser Werte in \@ref(eq:cv) ergibt sich:
...
Die einzelnen $z$-Werte für die Luftdruckmessungen ergeben sich wie in \@ref(tab:trans) dargestellt.
...
kable(df, "html", escape=F, align="rcr", caption="$z$-Transformation") %>%
...
<div class="rtip_content">In R kann eine empirische Verteilung mit dem Behfehl `scale()` $z$-transformiert werden.</div>
...
Da es sich um eine *theoretische* Verteilung handelt, ist die Normalverteilung zunächst insbesondere in Bezug auf die Grundgesamtheit interessant. Im Kontext der Grundgesamtheit wird das arithmetische Mittel mit $\mu$ ("Mü") und die Standardabweichung mit $\sigma$ ("Sigma") bezeichnet (s. \@ref(tab:param)).
...
") %>% kable("html", escape=F, caption="Bezeichnung von Parametern in Stichprobe und Grundgesamtheit") %>%
  scroll_box(box_css = "overflow-y: auto;")
```
...
Das Perzentil $x_p$ (engl. *percentile*) ist definiert als der Wert, unter dem der Anteil $p$ der Verteilung liegt. In [Sitzung 2](#einleitende-bemerkungen) haben wir also bereits den Median $x_{50\%}$ sowie die Angelpunkte $Q1=x_{25\%}$ und $Q3=x_{75\%}$ kennengelernt.
...
Für die Standardnormalverteilung finden sich die $p$-Werte für positive $z$ in der [Formelsammlung](Formelsammlung und Wertetabellen.pdf){target="_blank"}.^[Manchmal wird die Funktion $z_p \rightarrow P(z < z_p)$ für normalverteilte Werte auch mit $\Phi(z)$ bezeichnet [z.B. in @bahrenberg].]
...
Die [Formelsammlung](Formelsammlung und Wertetabellen.pdf){target="_blank"} gibt für $z$-Werte die Wahrscheinlichkeit ihrer Unterschreitung in ener Normalverteilung an. Diese Wahrscheinlichkeit kann notiert werden als $P(z<z_p)$.

Der [Formelsammlung](Formelsammlung und Wertetabellen.pdf){target="_blank"} können wir den Wert $P(z < 1,41) \approx 0,9207$ entnehmen. Die Wahrscheinlichkeit, dass der Luftdruck zu einem zufälligen Zeitpunkt weniger als 1015 hPA beträgt, ist somit 92,07%.

<div class="rtip">Softwarehinweis</div>
<div class="rtip_content">In R lässt sich die Unterschreitungswahrscheinlichkeit eines $z$-Werts mit dem Befehl `pnorm()` ermitteln.</div>
...
Die [Formelsammlung](Formelsammlung und Wertetabellen.pdf){target="_blank"} enthält keine $p$ für negative $z_p$. Da die Standardnormalverteilung jedoch um $z=0$ symmetrisch ist, gilt ganz allgemein:
...
<div class="rtip">Softwarehinweis</div>
<div class="rtip_content">Der Befehl `pnorm()` funktioniert auch mit negativen $z$-Werten.</div>
...
Dann können wir die entsprechende Wahrscheinlichkeit berechnen, indem wir wieder die Werte aus der [Formelsammlung](Formelsammlung und Wertetabellen.pdf){target="_blank"} einsetzen:
...
<div class="rtip">Softwarehinweis</div>
<div class="rtip_content">Das Perzentil für eine gegebene Unterschreitungswahrscheinlichkeit lässt sich in R mit `qnorm()` bestimmen.</div>
