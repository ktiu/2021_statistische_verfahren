...
kable(duty, "latex", booktabs=T, col.names = c("$x_i$", "$y_i$"), escape = F, longtable=F, linesep=c(""), caption = "\\label{tab:duty}Messwerte am Frankfurter Flughafen") %>% 
  kable_styling() %>% 
...
Graphisch ist diese lineare Regression in \@ref(fig:duty) dargestellt.

```{r duty, cache=T, fig.cap="Regressionslinie durch ein Streudiagramm"}
...
Graphisch sind die Residuen für unser Beispiel in \@ref(fig:res) dargestellt (positive Werte in grün, negative Werte in rot), tabellarisch in \autoref{tab:res}.

```{r res, cache=T, fig.cap="Graphische Darstellung der Residuen"}
model <- lm(duty$y~duty$x)
ggplot(duty,aes(x,y)) +
  geom_abline(intercept=model$coefficients[1],slope=model$coefficients[2], color=goethe_blue) +
  annotate("text", x=180, y=40, label=paste0("y%~~%3.35+0.114%.%x"), parse=T, color=goethe_blue) +
...
kable(duty, "latex", booktabs=T, col.names = c("$x_i$", "$y_i$", "$\\hat{y}_i\\approx3,35+0,114\\cdot x_i$", "$e_i=y_i-\\hat{y}_i$"), escape = F, longtable=F, linesep=c(""), caption = "\\label{tab:res}Residuen der Beispielwerte") %>% 
  kable_styling() %>% 
...
ex1[,3:4] <- NULL
kable(ex1, "latex", booktabs=T,  escape = F, longtable=F, linesep=c(""), col.names = c("$x_i$", "$y_i$"), format.args = list(digits=3)) %>% 
  kable_styling()
```
...
kable(ex3, "latex", booktabs=T,  escape = F, longtable=F, linesep=c(""), col.names = c("Vorbereitungszeit (min)", "Erreichte Punktzahl")) %>%
  kable_styling()
```
