...
Wir fragen uns, wie die Aufenthaltszeit von Passagieren am Frankfurter Flughafen mit dem Betrag zusammenhängt, den sie in den dortigen Geschäften ausgeben. Eine Zufallserhebung habe die Werte in \@ref(tab:duty) ergeben.

```{r}
duty <- read.table("img/8_dutyfree_dt")
kable(duty, "html", col.names = c("$x_i$", "$y_i$"), escape = F, longtable=F, linesep=c(""), caption = "\\label{tab:duty}Messwerte am Frankfurter Flughafen") %>%
...
Graphisch sind die Residuen für unser Beispiel in \@ref(fig:res) dargestellt (positive Werte in grün, negative Werte in rot), tabellarisch in \@ref(tab:res).
...
kable(duty, "html", col.names = c("$x_i$", "$y_i$", "$\\hat{y}_i\\approx3,35+0,114\\cdot x_i$", "$e_i=y_i-\\hat{y}_i$"), escape = F, longtable=F, linesep=c(""), caption = "\\label{tab:res}Residuen der Beispielwerte") %>%
