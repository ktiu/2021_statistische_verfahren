...
lectuR::klassentabelle_kum(tbbl) %>%
  kable_styling() %>%
...
```{r}
lectuR::klassentabelle_mittel(tbbl) %>%
  kable_styling() %>%
  scroll_box(box_css = "overflow-y: auto;")
```
...
```{r}
lectuR::klassentabelle_varianz(tbbl) %>%
  kable_styling() %>%
  scroll_box(box_css = "overflow-y: auto;")
```
...
```{r fig.height=5, fig.width=3, out.width='35%'}
...
ggplot(aes(y=y)) +
  ...
  geom_boxplot(fill = pubIHG::mustard_yellow) +
