...
Wir fragen uns, ob der jährliche Ertrag in einem bestimmten Anbaugebiet für Klebreis in Nordostthailand mit dem jährlichen Niederschlag zusammenhängt. Die erfassten Werte sind in \autoref{tab:reis} festgehalten ("Rai" ist ein [in Thailand übliches Flächenmaß](https://de.wikipedia.org/wiki/Rai_\(Einheit\))).

```{r, cache=T}
reis <- read.table("img/7_reis_dt")
reis <- cbind(data.frame(i=(1:10)), reis)
kable(reis, "latex", caption = "\\label{tab:reis}Niederschlag und Ertrag im Reisanbau", booktabs=T, linesep=c(""), longtable=F, col.names = c("$i$", "", "$x_i$", "$y_i$"), escape = F, row.names =F) %>% 
  kable_styling() %>%
...
kable(reis, "latex", align="r", caption = "\\label{tab:ricemath}Hilfstabelle für die Berechnung der Kovarianz", booktabs=T, linesep=c(""), longtable=F, col.names = c("$i$", "$x_i$", "$y_i$", "$(x_i-\\bar{x})$", "$(y_i-\\bar{y})$", "$(x_i-\\bar{x})\\cdot(y_i-\\bar{y})$"), escape = F, row.names =F) %>% 
  kable_styling() %>% row_spec(11, bold = T) %>% row_spec(10, hline_after = T)
```
...
```{r corrs, cache=T, fig.cap="Verschiedene Korrelationskoeffizienten", echo=FALSE, out.width='.26\\linewidth', fig.width=3, fig.height=3, fig.show='hold', fig.align='center'} 
...
In der Formel für den Korrelationskoeffizienten $r$ (\@ref(eq:cor)) werden die Standardabweichungen $s_x$ und $s_y$ benötigt. Es ist daher sinnvoll, die Hilfstabelle um die Quadrate der Differenzen (und deren Summen) zu erweitern (s. \autoref{tab:mathcor}).
...
sums[4:5] <- ""
reis <- rbind(reis, sums)
kable(reis, "latex", align="r", caption = "\\label{tab:mathcor}Hilfstabelle für die Berechnung des Korrelationskoeffizienten", booktabs=T, linesep=c(""), longtable=F, col.names = c("$i$", "$x_i$", "$y_i$", "$(x_i-\\bar{x})$", "$(y_i-\\bar{y})$", "$(x_i-\\bar{x})^2$", "$(y_i-\\bar{y})^2$"), escape = F, row.names =F) %>% 
  kable_styling() %>% row_spec(11, bold = T) %>% row_spec(10, hline_after = T)
```
...
kable(data.frame(ex$xa, ex$ya), "latex", booktabs=T, longtable=F, linesep=c(""), col.names = c("$x_i$", "$y_i$"), escape = F) %>% kable_styling()
```

b) Messreihe:

```{r}
kable(read.table("img/7_exercise_b_dt"),"latex", booktabs=T, longtable=F, linesep=c(""), col.names = c("$x_i$", "$y_i$"), escape = F) %>% kable_styling()
```
...
```{r}
kable(read.table("img/7_wasserhäuschen_dt"),"latex", booktabs=T, longtable=F, linesep=c(""), col.names = c("Entfernung (m)", "Umsatz (€/Tag)"), escape = F) %>% kable_styling()
```
