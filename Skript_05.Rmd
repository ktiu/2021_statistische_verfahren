...
Im Gegensatz zur Standardnormalverteilung (die wir für den $z$-Test benutzen) gibt es aber nicht nur eine $t$-Verteilung, sondern die Form der $t$-Verteilung hängt von so genannten Freiheitsgraden (engl. *degrees of freedom*) ab. Mit steigender Zahl der Freiheitsgrade nähert sich die $t$-Verteilung einer Standardnormalverteilung an (s. \autoref{fig:tdf}).
