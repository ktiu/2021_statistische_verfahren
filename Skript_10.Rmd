...
Grundlage sind bivariate Häufigkeiten, die in einer Kreuztabelle dargestellt werden können (s. \@ref(tab:zivi1)). Wie Kreuztabellen erstellt werden, haben wir bereits in Sitzung 9 behandelt.
...
```{r zivi2, fig.pos='H'}
zivixt <- crosstable(zivi, format="latex", varnames = F, expected = T, chisq = T, caption = "Kreuztabelle mit Erwartungswerten und Teilwerten für $\\chi^2$")
zivixt$df
```
...
Table: \label{tab:late}Häufigkeiten größerer Verspätungen im ÖPNV
...
\\makecell[tr]{459\\\\(421,2)\\\\\\color{goethe_blue}{3,392}} \\makecell[tr]{409\\\\(421,2)\\\\\\color{goethe_blue}{0,353}} \\makecell[tr]{414\\\\(421,2)\\\\\\color{goethe_blue}{0,123}} \\makecell[tr]{387\\\\(421,2)\\\\\\color{goethe_blue}{2,777}} \\makecell[tr]{437\\\\(421,2)\\\\\\color{goethe_blue}{0,593}} 2106") %>%
  kable("latex", escape=F, longtable=F, col.names=c("Montag", "Dienstag", "Mittwoch", "Donnerstag", "Freitag", " "), caption = "Tabelle für den $\\chi^2$-Anpassungstest") %>%
...
draw.curve <- function(interval, colorf, xlabel, curve, n) {
  ...
  p1 <- ggplot(data = data.frame(x = interval), aes(x)) +
...
m <- as.data.frame(m)
knitr::kable(m, "html", escape = F, align = "r") %>%
