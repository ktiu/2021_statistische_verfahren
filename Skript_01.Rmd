...
```{r skalen}
c("Nominalskala", "Postleitzahl", "Gleichheit, Verschiedenheit", "Modus",
  ...
  as_tibble %>%
  kable("html", col.names= c("Skalenart", "Beispiel", "Mögliche Aussagen", "gültige Lagemaße"), caption="Die vier wichtigsten Skalenniveaus.") %>%
...
<div class="rtip_content">In R lässt sich mit dem Befehl `table()` eine einfache Häufigkeitstabelle aus Rohdaten erstellen.</div>
...
kable(df, "html", escape=F, caption = "Tabelle mit kumulierten Häufigkeiten", linesep = "" ,align = "lrrrr") %>%
  scroll_box(box_css = "overflow-y: auto;")
```

### Stabdiagramme

Die so ermittelten Häufigkeiten lassen sich als Stabdiagramm (auch Säulen-, Streifen-, Balkendiagramm, engl. *bar chart*) darstellen (s. Abbildung \@ref(fig:stab)).

<div class="rtip">Softwarehinweis</div>
<div class="rtip_content">In R lautet der Standardbefehl zur Erstellung eines Stabdiagramms `barplot()`.</div>

```{r stab, fig.cap="Stabdiagramm"}
bar <- data.frame(xs)
bar$xs <- as.factor(xs)
ggplot(bar, aes(x=bar$xs)) +
...
knitr::kable(df, "html", escape=F, linesep="", caption = "Häufigkeitstabelle mit klassierten Werten",align = "lrrrr") %>%
  scroll_box(box_css = "overflow-y: auto;")
```
...
Die Darstellung erfolgt in so genannten Histogrammen (engl. *histogram*). Abbildung \@ref(fig:hist) enthält ein Beispiel für ein Histogramm.

<div class="rtip">Softwarehinweis</div>
<div class="rtip_content">In R können Histogramme mit `hist()` erstellt werden.</div>

```{r hist, fig.cap="Histogramm"}

ggplot(data=trees, aes(x=trees$Girth)) +
...
Polygone von Häufigkeitsverteilungen (insbesondere in geglätteter Form) ergeben Annäherungen an so gennannte Dichtefunktionen (engl. *density functions*). Diese lassen sich mit Attributen (uni-/bimodal, schmal-/breitgipflig, etc.) beschreiben, wie in Abbildung 4 veranschaulicht.

![Merkmale von Verteilungen [aus: @bortz: 42]](img/shapes.png){height=700}
...
```{r, comment=NA, results='asis'}
cat('`', paste(scan("img/1_klimastunden"), collapse = " "),'`')
```
