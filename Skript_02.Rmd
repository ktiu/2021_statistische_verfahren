...
colnames(table) <- th
knitr::kable(table, "latex", booktabs=T, escape=F, caption = "Die wichtigsten Maßzahlen") %>% kable_styling(latex_options = c("hold_position")) 
```
...
matrix(xs, nrow = 1, dimnames = list(c(), sprintf("$x_{%s}$", 1:14)))%>% as_tibble %>%
  kable("latex", booktabs=T, escape=F, caption="Beispielverteilung") %>%
...
matrix(sort(xs), nrow = 1, dimnames = list(c(), sprintf("$x_{(%s)}$", 1:14)))%>% as_tibble %>%
  kable("latex", booktabs=T, escape=F, caption="Sortierte Wertereihe") %>%
  kable_styling(latex_options = c("hold_position"))
```
...
knitr::kable(df, "latex", escape=F, booktabs=T, align="r", caption = "Häufigkeiten der Beispielverteilung", linesep=c("")) %>%
  kable_styling(latex_options = c("HOLD_position"))
```
...
kable(df, "latex", booktabs=T, escape = F, linesep=c(""), caption="Häufigkeitstabelle zur Berechnung der Varianz") %>% 
...
```{r box, fig.height=5, fig.width=3, out.width='.3\\linewidth', fig.cap="Boxplot der Beispielverteilung"}
...
In Australien betrug die durchnittliche Niederschlagsmenge in den 1970er und 80er Jahren ^[Auszug aus dem Datensatz `bomsoi` in @daag]:
\nopagebreak
```{r}
DAAG::bomsoi[71:90,c(1,15)] %>%
  as_tibble(rownames = NULL) %>%
  kable("latex", booktabs=T, linesep=c(""), col.names = c("Jahr", "Niederschlag (mm)")) %>%
